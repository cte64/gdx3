...
public class TextComponent extends Component {
    ...
    public void update(Entity ent) {
        ...
        if(vertical.equals("bottom")) yPos = (int)(ent.y_pos + Engine.get().getAssets().getTextBounds(text, currentFontSize).y)+ yOffset;
        if(vertical.equals("top")) yPos = (int)(ent.y_pos + ent.getHeight() - Engine.get().getAssets().getTextBounds(text, currentFontSize).y)+ yOffset;

        //horizontal
        if(horizontal.equals("center")) xPos = (int)(ent.x_pos + ent.getWidth()/2 - Engine.get().getAssets().getTextBounds(text, currentFontSize).x/2  ) + xOffset;
        if(horizontal.equals("left")) xPos = (int)(ent.x_pos) + xOffset;
        if(horizontal.equals("right")) xPos = (int)(ent.x_pos + ent.getWidth() - Engine.get().getAssets().getTextBounds(text, currentFontSize).x) + xOffset;
    }
